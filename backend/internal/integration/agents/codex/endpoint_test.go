...
func TestArgsCarryTheEndpointProviderTable(t *testing.T) {
	...
	// The app-server subcommand stays first so codex parses the overrides as
	// configuration for it rather than as a prompt.
	if len(args) == 0 || args[0] != "app-server" {
		t.Errorf("first arg = %#v, want the app-server subcommand", args)
	}
}
