...
| [26-agent-endpoints.md](../02-workspaces/26-agent-endpoints.md) | Third-party agent endpoints: pointing one chat's CLI at a vendor's own published compatibility endpoint with the operator's key, per-run env/config isolation, vault-backed key resolution, the red "not Anthropic" badge, and the in-container test |
