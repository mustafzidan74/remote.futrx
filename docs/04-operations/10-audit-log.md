...
| `agent.run.` | `start`, `cancel` — `meta` carries `provider`, `chatId`, `projectId`, the scheduled task/run ids for unattended turns, and `endpointId` when the turn ran against a [third-party agent endpoint](../02-workspaces/26-agent-endpoints.md) |
...
| `settings.agent-endpoint.` | `create`, `update`, `delete`, `test` — the third-party agent endpoint register; `meta` carries the CLI and the enabled state, never an API key ([Agent endpoints](../02-workspaces/26-agent-endpoints.md)) |
| `self-update.` | `trigger` |
